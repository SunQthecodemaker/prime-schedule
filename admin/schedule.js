// admin/schedule.js — 엑셀 스타일 스케줄 관리 (jSpreadsheet CE v5)
...
function createSheet(el, rowList, dateList, data) {
  ...
  // jSpreadsheet CE v5: worksheets 배열 + 콜백은 최상위
...
function onMoveRow(ws, from, to) {  // v5: 첫 파라미터 worksheet
  const [m]=rows.splice(from,1); rows.splice(to,0,m);
  saveLayout().catch(e=>console.warn('레이아웃 저장 실패:',e));
}

function ctxMenu(ws, x, y, e, items) {  // v5: 첫 파라미터 worksheet
